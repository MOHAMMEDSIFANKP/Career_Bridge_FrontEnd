...
function Notifications() {
  ...
  const Isread = async (id, path,read) => {
    ...
    try {
      ...
      const res = await NotificationRead(data, id);
      if (res.status === 200) {
  ...
  return (
    <>
      <div
        className="grid grid-rows-[5rem,
      1fr]"
      >
        <div>
          <NavbarDefault />{" "}
        </div>
        <div className="ps-6  container mx-auto mt-6 flex text-3xl items-center">
          <svg
        ...
        <div className="overflow-y-auto mt-7  container mx-auto rounded-xl  h-[48rem]">
          {NotificationList.length > 0 ? (
            NotificationList.map((notify, index) => (
              <div
                key={index}
                className={`border rounded-xl mx-3 my-3 flex items-center justify-between h-16 shadow cursor-pointer ${
                  notify.is_read ? "" : "bg-purple-50"
                }`}
              >
                <div className="flex">
                  <svg
            ...
            <div className="h-full bg-purple-50 flex justify-center items-center">
              <p className="font-bold text-3xl">Notification is empty</p>
            </div>
          )}
        </div>
      </div>

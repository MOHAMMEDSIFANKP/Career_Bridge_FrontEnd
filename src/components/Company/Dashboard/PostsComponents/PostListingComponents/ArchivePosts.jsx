      ...
      {!view.view ? (
        <>
          ...
          {Searcheddata.length > 0 ? (
            Searcheddata.filter((Post) => Post.is_deleted).map(
              (Post, index) => (
                <div
                  key={index}
                  className="grid grid-rows-[5rem,1fr] cursor-pointer bg-red-50 hover:bg-gray-100 text-gray-600 border mx-8 mt-4  rounded-2xl shadow"

...
import { InviteRejectedUsers, InviteUserListUserside, IviteAcceptedUsers } from "../../../../services/companyApi";
function InvitesList() {
  ...
  const handleSearch = async (searchTerm) => {
    ...
    try {
      handleLoading()
      const res = await InviteUserListUserside(UserInfo.userinfoid, searchTerm);
      setPosts(res.data);
  ...
  // Accept invitations
  const AcceptedInvitations = async(id) =>{
    try {
      handleLoading()
      const res = await IviteAcceptedUsers(id)
      if (res.status === 202){
        GetInviteusersList()
  ...
  const RejectedInvitations = async (id)=>{
    try {
      handleLoading()
      const res = await InviteRejectedUsers(id)
      console.log(res);
      if (res.status === 200){
        GetInviteusersList()
        toast.success(res.data.message)
  ...
  // Fech data backend
  async function GetInviteusersList() {
    try {
      handleLoading()
      const res = await InviteUserListUserside(UserInfo.userinfoid, "");
      setPosts(res.data);
      setSearcheddata(res.data.results);
      handleLoading()
    } catch (error) {
  ...
  const { data, isLoading, isError } = useQuery("requests", GetInviteusersList);
  if (isLoading) {
    return <Loader />;
  }
  ...
  return (
   ...
   {loading && <Loader />}
   <ToastContainer />
    <div className="border container rounded-xl shadow h-[46rem]">
      {!view.view ? (
        <>
            ...
            >
              ...
              <div className="grid grid-rows-[1fr,1rem]">
                <div className="flex justify-center items-center gap-4">
                 ...
                 <button className="bg-green-400 font-bold text-white px-3 py-1 rounded-xl" onClick={()=>AcceptedInvitations(Post.id)}>Accept</button>
                 <button className="bg-red-300 font-bold text-white px-3 py-1 rounded-xl" onClick={()=>RejectedInvitations(Post.id)}>Reject</button>
                </div>
              </div>
            </div>
          ))}

...
import { EducationDetails } from "../../../../services/userApi";

const EditEducationsModal = ({ isOpen, onClose, id }) => {
  ...
  useEffect(() => {
    ...
    const educationToEdit = Education.find(
      (education, index) => index === id.index
    );
  ...
  const ConfirmButton = async () => {
    if (Validation()) {
      try {
        const res = await EducationDetails(Form, id.id);
        toast.success("Education Updated");
        dispatch(
          EditEducation({ index: id.index, updatedEducation: res.data })
        );
      } catch (error) {}
      onClose();
    }

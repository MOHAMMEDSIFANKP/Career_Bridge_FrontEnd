...
// Service
import { DeleteExperienceDetails } from "../../../../services/userApi";

const DeleteExperienceModal = ({ isOpen, onClose, id }) => {
  const dispatch = useDispatch();
  const { experiences } = useSelector((state) => state.user);
  const experienceToDlt = experiences.find((experience, index) => index === id.index);
  const ConfirmButton=async()=>{
    try{
        const res = await DeleteExperienceDetails(id.id)
        dispatch(DeteteExperience(id.index))
    }catch(error){

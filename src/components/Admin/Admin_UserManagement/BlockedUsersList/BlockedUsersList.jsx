...
import { BlockUsersList, UserBlockUnBlock } from "../../../../services/adminApi";
...
function BlockedUsersList() {
  ...
  const handleSearch = async (searchTerm) => {
    ...
    handleLoading();
    try {
      const res = await BlockUsersList(searchTerm);
      console.log(res);
  ...
  const BlockUnblock = async (value, id) => {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        const res2 = await BlockUsersList(Search);
  ...
  async function GetUsersList() {
    try {
      const res = await BlockUsersList(Search);
      setFilteredUserList(res.data);
      setSearcheddata(res.data.results);
    } catch (error) {
      console.log(error);

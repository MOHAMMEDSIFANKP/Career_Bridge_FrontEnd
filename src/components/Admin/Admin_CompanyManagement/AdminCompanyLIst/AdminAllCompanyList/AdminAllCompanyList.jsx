...
import { AllCompanyList,VerifyAndBlock } from "../../../../../services/adminApi";
import { Chip } from "@material-tailwind/react";

function AdminAllCompanyList() {
  ...
  const handleSearch = async (searchTerm) => {
    ...
    handleLoading();
    try {
      const res = await AllCompanyList(searchTerm);
      console.log(res);
  ...
  const CompanyVerifyAndBlock = async (value, id) => {
    ...
    try {
      ...
      if (res.status === 200) {
        const Search = ''
        const res2 = await AllCompanyList(Search);

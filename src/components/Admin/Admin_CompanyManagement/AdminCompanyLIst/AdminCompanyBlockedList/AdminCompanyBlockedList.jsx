...
import { CompanyBlockedList, CompanyVerifiedList,VerifyAndBlock } from "../../../../../services/adminApi";
import { Chip } from "@material-tailwind/react";

function AdminCompanyBlockedList() {
  ...
  const handleSearch = async (searchTerm) => {
    ...
    handleLoading();
    try {
      const res = await CompanyBlockedList(searchTerm);
      console.log(res);
  ...
  const CompanyVerifyAndBlock = async (value, id) => {
    ...
    try {
      ...
      if (res.status === 200) {
        const Search = ''
        const res2 = await CompanyBlockedList(Search);
  ...
  async function GetCompanyList() {
    try {
      const res = await CompanyBlockedList(Search);
      setFilteredCompanyList(res.data);
      setSearcheddata(res.data.results);
    } catch (error) {
      console.log(error);
...
export default AdminCompanyBlockedList;

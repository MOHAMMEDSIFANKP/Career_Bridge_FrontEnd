...
import { UnblockSkillsModal } from "./UnblockSkillsModal";

function BlockedSkillsList() {
  ...
  const handleSearch = async (searchTerm) => {
    ...
    try {
      const res = await BlockedSkillsLists(searchTerm);
      setPosts(res.data);
  ...
  async function GetSkillslist() {
    try {
      handleLoading();
      const Search = "";
      const res = await BlockedSkillsLists(Search);
      setPosts(res.data);
      setSearcheddata(res.data.results);
      handleLoading();
    } catch (error) {
  ...
  const { data, isLoading, isError } = useQuery("GetSkillslist", GetSkillslist);
  if (isLoading) {
    return <Loader />;
  }
  ...
  return (
    <>
      ...
      {Searcheddata.map((skills, index) => (
        <div
          key={index}
          className="shadow border mx-5 rounded-xl bg-red-50 my-4 grid grid-cols-[1fr,6rem] py-4"
        >
...
export default BlockedSkillsList;

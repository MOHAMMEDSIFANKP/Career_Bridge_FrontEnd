...
import { BlockSkillsModal } from "./BlockSkillsModal";

function SkillsList() {
  const [Form, setForm] = useState({ id: "", skills:''});
  ...
  const SelectedItem = (id, index) => {
    ...
    setSelectedPost(sel);
    setForm({id:sel.id,skills:sel.skills})
  ...
  const handleSearch = async (searchTerm) => {
    ...
    try {
      const res = await SkilLslist(searchTerm);
      setPosts(res.data);
  ...
  const EditJobTitle = async () => {
    if (Form.skills.trim() === "") {
      ...
      try {
        handleLoading();
        const res = await SkillsDetails(Form, Form.id);
        if (res.status === 200) {
          toast.success(` ${res.data.skills} Edited success fully`);
          const Search = "";
          const res2 = await SkilLslist(Search);
          setPosts(res2.data);
  ...
  async function GetJobTitle() {
    try {
      handleLoading();
      const Search = "";
      const res = await SkilLslist(Search);
      setPosts(res.data);
      setSearcheddata(res.data.results);
      handleLoading();
    } catch (error) {
  ...
  return (
    <>
      ...
      {Searcheddata.map((skills, index) => (
        <div
          key={index}
          className="shadow border mx-5 rounded-xl my-4 grid grid-cols-[1fr,6rem] py-4"
        >
          {view.view && view.id === skills.id ? (
            <>
              <div className="flex justify-center items-center gap-5 ms-5">
                <input
                  type="text"
                  onChange={(e) =>
                  ...
                  className="border w-4/6 px-2 py-2  rounded-lg text-black placeholder-gray-700 text-sm focus:border-purple-500 focus:outline-none focus:ring focus:ring-purple-100 border-gray-400"
                />
              </div>
            ...
            <>
              ...
              <div className="flex justify-end mx-3">
                <svg
                  onClick={() => SelectedItem(skills.id, index)}
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 24 24"
                  strokeWidth={1.5}
                ...
                <svg
                  onClick={() => {
                    handleOpenDelete(), SelectedItemDlt(skills.id, index);
                  }}
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 24 24"
